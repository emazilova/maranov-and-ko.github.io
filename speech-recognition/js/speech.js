...
function getCurrenciesFromInputs() {
    ...
    if(!sum || isNaN(sum) || !currencyFrom || !currencyTo) {
        return;
    }
...
function getCurrenciesFromVoice(text) {
    ...
    if (foundWords === null || foundWords[2] === undefined || foundWords[3] === undefined || foundWords[4] === undefined) {
        ...
        if (!currencyFrom) {
            result.innerHTML += `Невозможно сконвертировать данную валюту: ${foundWords[3]}.<br><br>`;
        } else if(!currencyTo) {
            result.innerHTML += `Невозможно сконвертировать данную валюту: ${foundWords[4]}.<br><br>`;
        } else {
            ...
            result.innerHTML += `Результат: ${convertedValue + CURRENCY_INFO[currencyTo].symbol}<br><br>`;
        }
    }
}
